...
import pprint as pp
import hgnn.utils.hypergraph_utils as hgut
from hgnn.models import HGNN
from hgnn.config import get_config
from hgnn.datasets import load_feature_construct_H
import numpy as np

#sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))


def train(args, fts, lbls, idx_train, idx_test):
    lbls = lbls[:, np.newaxis]
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    print(os.getcwd())
    cfg = get_config(os.path.join(os.getcwd(), 'competitors', 'competitors_setups', 'HGNN', 'hgnn', 'config', 'config.yaml'))

    # initialize data
    cfg.update(args)
    fts, lbls, idx_train, idx_test, H = \
        load_feature_construct_H(fts, lbls, idx_train, idx_test,
                                m_prob=cfg['m_prob'],
                                K_neigs=cfg['K_neigs'],
    ...
    model_ft, preds = train_model(model_ft, criterion, optimizer, schedular, cfg['max_epoch'], print_freq=cfg['print_freq'])
