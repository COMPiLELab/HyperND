...
from appnp.appnp_layer import APPNPModel

class APPNPTrainer(object):
    """
    Method to train PPNP/APPNP model.
    """
    def __init__(self, args, graph, features, target, train_inds, test_inds, val_inds):
        """
        ...
        self.target = target
        self.train_nodes = train_inds
        self.test_nodes = test_inds
        self.validation_nodes = val_inds
        self.create_model()
        ...
        for _ in iterator:
            ...
            if accuracy >= self.best_accuracy:
                ...
                if self.step_counter > self.args.early_stopping_rounds:
                    iterator.close()
                    break
        return self.model(self.feature_indices, self.feature_values).max(dim=1)[1]
